...
const VariantButton = ({ variant }: { variant: ClipVariant }) => {
  const { setSelectedVariant, setStep } = useCreateClipContext()
  ...
  return (
    <button
      onClick={onClick}
      className={`${variant.borderStyle} ${variant.textStyle} ${variant.bgStyle} transition-all duration-300 w-12 h-12 bg-background-white border-2 hover:text-foreground-white rounded-full flex justify-center items-center`}
    >
